...
public class MovieServiceResource {
    ...
    public List<CatalogItem> getCatalogResourcesMovies(@PathVariable("userId") String userId) {
        ...
        UserRating userRating = restTemplate.getForObject("http://movi-ratings-service/api/rating/users/" + userId, UserRating.class);

        return userRating.getUserRating().stream().map(rating -> {
                    //foreach movie IDs, call movie info and get its details
                    MovieItem movieItem = restTemplate.getForObject("http://movies-info-service/api/info/" + rating.getMovieId(), MovieItem.class);
